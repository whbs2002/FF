...
def density_plots(yearly,player_identity, ppr=True,years=(2019,2025),limit = 0.0):
    points = ""
    positions = ['QB','WR','RB','TE']
    if ppr: points = 'fantasy_points_ppr' 
    ...
    yearly = yearly.merge(player_identity, on='player_id')
    ...
    for name,group in grouped:
        labels.append(name)
        data.append(group[points].to_numpy())
    print(labels)
    fig = ff.create_distplot(data,labels,bin_size=10)
    fig.write_html('figures/par_hist.html')
    fig.write_image('figures/par_hist.png')
...
def main():
    ...
    file.close()
    density_plots(yearly,identity,ppr=False,limit=100.0)
